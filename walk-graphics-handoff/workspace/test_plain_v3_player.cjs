...
const html=fs.readFileSync('plain-walk-v3/plain-walk-player.html','utf8'),els={};let downloaded=null,lastBlob=null;
...
for(const id of ['front','side','partsFront','partsSide']){const c=createCanvas(id.startsWith('parts')?720:384,id.startsWith('parts')?540:512);els[id].canvas=c;els[id].width=c.width;els[id].height=c.height;els[id].getContext=()=>c.getContext('2d');}
...
setTimeout(async()=>{try{
 ...
 fs.writeFileSync('plain-v3-player-side-check.png',png('side'));fs.writeFileSync('plain-v3-player-parts-check.png',png('partsSide'));
 ...
 els.reset.click();assert(els.sideNear.textContent.includes('42°'));
 ...
 els.save.click();assert.equal(downloaded,'walk-pose-06.json');const saved=JSON.parse(await lastBlob.text());assert.equal(saved.side.frame,5);assert.equal(saved.side.arms.near.elbow_flex,42);
