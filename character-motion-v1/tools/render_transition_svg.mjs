...
const lib=library(),src={...R().poses,...S().poses},meta={...R().motions,...S().motions};
...
for(const [id,m] of Object.entries(lib.motions)){
 ...
 for(const d of [v,second]){
  const A=a==='stand'?[tag(frameOf({...NEUTRAL,ms:200},d),'（前）立ち姿勢 standing')]:(()=>{const F=src[a][d],lp=meta[a].loop,i=lp?0:F.length-1;return(lp?[F[F.length-1],F[0]]:[F[i-1],F[i]]).filter(Boolean).map(f=>tag(f,`（前）${a}`))})();
  const B=b==='stand'?[tag(frameOf({...NEUTRAL,ms:200},d),'（次）立ち姿勢 standing')]:src[b][d].slice(0,2).map(f=>tag(f,`（次）${b}`));
