...
function drawEnemy(e=state.enemy){
 ...
 ctx.globalAlpha=(e.stun>0?.72:1)*(e._dimA??1);   // v0.43: _dimA/_dimF - a fallen monster lying dim (polish-v014.js)
 ...
 if(custom){ctx.filter=e._dimF||'none';if(window.Game5PixelArt?.has(custom))ctx.imageSmoothingEnabled=false;let cw=size*.86,ch=cw*custom.naturalHeight/custom.naturalWidth;if(ch>size*1.05){cw*=size*1.05/ch;ch=size*1.05}ctx.drawImage(custom,-cw/2,size*.3-ch,cw,ch)}
