...
function makePose(view,frame,config,assets,params={}){
 ...
 const neck=spine.neck,head=neck.slice();
 ...
 function arm(side,k){
  ...
  if(view==='side'){
   ...
   elbow=[shoulder[0]-lateral*.9,shoulder[1]+elbowSag[1]-.08*elbowSag[0]];
   wrist=[shoulder[0]+lateral*(.3+.025*wristSag[0]),shoulder[1]+wristSag[1]-.08*wristSag[0]];
   drawHandAngle=lateral*7;handSy=clamp(Math.cos(rad(handAngle)),.64,1);
  }
 ...
 for(const leg of sortedLegs)if(leg.foreground)ops.push(...leg.ops);
 ops.push(part('upper_body','upper_body',waist,lean,1,1,'waist'),part('neck','neck',add(neck,[0,4]),lean,1,1,'cervical'));
