...
- 走行GIF：[正面](sister-run-front.gif)／[右](sister-run-right.gif)／[左](sister-run-left.gif)。明るい背景へ合成した8コマのループ。

確認画像は [build_v7_review.py](../tools/build_v7_review.py) でゲーム用の原寸コマから組み立てる。比較元は `before-v6/walk/{front,right,back,left}/00.png` に退避した旧版4枚。[入力画像のハッシュと生成物一覧](review-build-report.json)を保存する。比較用の背景合成以外に、原画の描き足しやコマの変形は行わない。
