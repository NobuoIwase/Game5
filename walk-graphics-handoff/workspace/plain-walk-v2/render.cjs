...
(async()=>{
 ...
 for(const v of ['front','side'])for(const [n,a] of Object.entries(assets[v]))images[v][n]=await loadImage(path.join(root,a.file));
 ...
 fs.writeFileSync(path.join(root,'rig.json'),JSON.stringify({schema:'plain-walk-2',config,assets,frames:poses},null,2));
 const data={config,assets,images:{front:{},side:{}}};
 for(const v of ['front','side'])for(const [n,a] of Object.entries(assets[v]))data.images[v][n]='data:image/png;base64,'+fs.readFileSync(path.join(root,a.file)).toString('base64');
 ...
 console.log('Rendered 16 frames from the shared joint rig.');
