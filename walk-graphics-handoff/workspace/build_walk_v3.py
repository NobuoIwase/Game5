...
def pose_side(f):
    ...
    ops+=leg('leg_far',4)
    ops+=[op('arm_far',*far_shoulder,(8,3),5-swing)]
    ops+=[op('hair',neck[0]-14,neck[1]-61,(42,0),lag*.5),
    ...
    ops += [op('skirt',104,144+b*.65,(40,0),lag*.35),
            ...
            op('arm_near',*near_shoulder,(8,3),-10+swing),
            op('head',*neck,(56,82),0)]
    return ops
...
preview[0].save(ROOT/'walk-check-v3.png')
...
(ROOT/'player-data-v3.json').write_text(json.dumps(DATA,separators=(',',':')))
template=ROOT/'player-template-v3.html'
if template.exists():
    (OUT/'lumina-walk-player.html').write_text(template.read_text().replace('__DATA__',json.dumps(DATA,separators=(',',':'))))
print(json.dumps({'output':str(OUT),'part_count':sum(len(p) for p in PARTS.values()),'frame_count':16,'preview':str(ROOT/'walk-check-v3.png')}))
