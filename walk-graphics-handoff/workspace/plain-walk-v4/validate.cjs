...
for(const view of ['down_right','up_right','back'])for(let f=0;f<8;f++){
 ...
 for(const side of ['far','near']){
  ...
  const bottom=foot.position[1]+rig.soleOffset(A[view][foot.asset],foot.angle)*foot.scale[1];
  const expected=a.ground_y-a.lift,e=Math.abs(bottom-expected);assert(e<eps,'Projected contact/lift mismatch');
 ...
 const body=p.ops.find(o=>o.id==='upper_body'),socket=A[view].upper_body.neck_socket,x=socket[0]-body.pivot[0],y=socket[1]-body.pivot[1],t=body.angle*Math.PI/180;
