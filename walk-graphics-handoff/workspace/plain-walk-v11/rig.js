...
function groundSupport(asset,a,slope=0){
 const c=Math.cos(rad(a)),s=Math.sin(rad(a)),[px,py]=asset.pivot;
 return Math.max(...asset.opaque_corners.map(([x,y])=>{
  const dx=c*(x-px)-s*(y-py),dy=s*(x-px)+c*(y-py);return dy-slope*dx;
 }));
}
function makePose(view,frame,config,assets,params={}){
 if(!['front','side'].includes(view))return makeTurnedPose(view,frame,config,assets,params);
...
function makeTurnedPose(view,frame,C,assets,params={}){
 const V=C[view],A=assets[view],P={...C.parameters,...params},f=((frame%8)+8)%8;
 const sagittal=makePose('side',f,C,assets,params),s=Math.sin(rad(V.yaw)),c=Math.cos(rad(V.yaw));
 const bob=sagittal.body_bob,sway=C.front.sway[f]*Math.abs(c),rootX=V.root_x+sway;
 const hipY=C.body.hip_y+bob,waist=[rootX,C.body.waist_y+bob],lean=C.side.lean_degrees*s;
 const depth=(l,z)=>c*z-s*l;
 const project=(l,y,z)=>[rootX+c*l+s*z,y];
 const around=(x,y)=>[waist[0]+x*Math.cos(rad(lean))-y*Math.sin(rad(lean)),waist[1]+x*Math.sin(rad(lean))+y*Math.cos(rad(lean))];
 const joints={},chains=[],arms={},ops=[];
 const node=(id,parent,p,meta={})=>(joints[id]={parent,position:p.slice(),...meta},p);
 const part=(asset,id,p,rotation=0,sx=1,sy=1,parent=null)=>({id,asset,parent,position:p.slice(),angle:rotation,scale:[sx,sy],pivot:A[asset].pivot.slice()});
 const joint=(asset,id,p,parent)=>part(asset,id,p,0,1,1,parent);
 const mirrored=side=>view==='back'?side==='near':side==='far';
 function segment(asset,id,a,b,parent){
  const mirror=['upper_arm','forearm'].includes(asset)&&mirrored(id.endsWith('_near')?'near':'far')?-1:1;
  return part(asset,id,a,angle(a,b),mirror,Math.hypot(b[0]-a[0],b[1]-a[1])/A[asset].length,parent);
 }
 node('root',null,[rootX,hipY]);
 let parent='root';
 for(const id of ['sacrum','lumbar','thorax']){
  const [z,y]=C.body.spine_side[id];node(id,parent,project(0,hipY+y,z));parent=id;
 }
 node('waist','lumbar',waist);
 // Diagonal C7 starts between the shoulders; the artwork already includes
 // the lower neck. The skull mount is separately registered for each view.
 const socket=A.upper_body.cervical_socket||A.upper_body.neck_socket,pivot=A.upper_body.pivot;
 const cervical=node('cervical','thorax',around(socket[0]-pivot[0],socket[1]-pivot[1]));
 const neck=node('neck','cervical',add(cervical,V.neck_tip_offset));
 const head=node('head','neck',add(neck,V.head_offset));
 function leg(side){
  const l=(side==='near'?-1:1)*C.front.hip_half_width;
  const h0=sagittal.joints['hip_'+side],k0=sagittal.joints['knee_'+side],a0=sagittal.joints['ankle_'+side],k=h0.phase;
  const kz=k0.position[0]-C.side.root_x,az=a0.position[0]-C.side.root_x;
  const dh=depth(l,0),dk=depth(l,kz),da=depth(l,az),footAsset=V.foot_assets[k];
  // The side rig defines lift and extension; yaw only changes projection.
  const lift=k<=4?0:Math.max(0,C.ground_y-a0.position[1]-soleOffset(assets.side.foot,a0.foot_angle));
  const fa=C.gait.foot_angle[k]*s*(V.foot_pitch_scale??.32),groundSlope=V.foot_ground_slope??0;
  const footScale=clamp(1+da*.0015,.94,1.035);
  const floor=V.ground_center+V.ground_depth*da;
  const ay=floor-lift-groundSupport(A[footAsset],fa,groundSlope)*footScale;
  const h=project(l,hipY+V.ground_depth*dh,0);
  const t=(k0.position[1]-h0.position[1])/(a0.position[1]-h0.position[1]);
  const knee=project(l,h[1]+(ay-h[1])*t+V.ground_depth*c*(kz-az*t),kz);
  const ankle=project(l,ay,az);
  node('hip_'+side,'root',h,{phase:k,inside_pelvis:true,depth:dh,thigh_depth:dk});
  node('knee_'+side,'hip_'+side,knee,{depth:dk,flex:k0.flex,sagittal_position:k0.position,occluded_by:da>dh?'shin':'thigh'});
  node('ankle_'+side,'knee_'+side,ankle,{depth:da,contact:k<=4,foot_angle:fa,foot_pitch:C.gait.foot_angle[k],foot_asset:footAsset,ground_y:floor,ground_slope:groundSlope,lift,sagittal_position:a0.position});
  const thigh=segment('thigh','thigh_'+side,h,knee,'hip_'+side),shin=segment('shin','shin_'+side,knee,ankle,'knee_'+side);
  const cap=joint('knee','knee_joint_'+side,knee,'hip_'+side);cap.scale[1]=clamp(1-k0.flex/180,.62,1);
  const group=[joint('hip','hip_joint_'+side,h,'root')];
  group.push(da>dh?thigh:shin,cap,da>dh?shin:thigh,joint('ankle','ankle_joint_'+side,ankle,'knee_'+side),part(footAsset,'foot_'+side,ankle,fa,footScale,footScale,'ankle_'+side));
  chains.push(['hip_'+side,'knee_'+side,'ankle_'+side]);
  return {ops:group,foreground:(dh+dk)/2>1,depth:(dh+dk+da)/3};
 }
 function arm(side){
  const sign=side==='near'?-1:1,l=sign*C.front.shoulder_half_width,k=side==='near'?f:(f+4)%8;
  const upper=C.arms.upper_angle[k]*P.arm_swing,flex=clamp(26+(C.arms.elbow_flex[k]-26)*P.elbow_variation,6,58);
  const wristFlex=C.arms.wrist_flex[k]*(V.wrist_flex_scale??1),lower=upper-flex,handAngle=lower+wristFlex;
  const es=down(upper,C.lengths.upper_arm),ws=add(es,down(lower,C.lengths.forearm));
  const z0=side==='near'?0:-2;
  const sy=waist[1]+C.body.shoulder_y+.10*depth(l,z0);
  const el=sign*(C.front.shoulder_half_width+C.front.arm_clearance.elbow+.04*Math.max(0,es[0]));
  const wl=sign*(C.front.shoulder_half_width+C.front.arm_clearance.wrist+.035*ws[0]);
  let shoulder=project(l,sy,z0),elbow=project(el,sy+es[1]-.08*c*es[0],z0+es[0]),wrist=project(wl,sy+ws[1]-.08*c*ws[0],z0+ws[0]);
  if(A.upper_body.shoulder_sockets){
   const q=A.upper_body.shoulder_sockets[side],socket=around(q[0]-pivot[0],q[1]-pivot[1]),delta=[socket[0]-shoulder[0],socket[1]-shoulder[1]];
   shoulder=socket;elbow=add(elbow,delta);wrist=add(wrist,delta);
  }
  const ds=depth(l,z0),de=depth(el,z0+es[0]),dw=depth(wl,z0+ws[0]);
  let ha=handAngle*s+sign*c*(4-.18*wristFlex),hsy=1-Math.abs(c)*(1-clamp(Math.cos(rad(handAngle)),.64,1));
  if(V.hand_projection==='forearm'){
   // Project a direction vector following the forearm; multiplying a 2-D
   // angle by yaw loses the lateral wrist slope in a rear view.
   const hd=down(handAngle,7),hl=(wl-el)/C.lengths.forearm*7*.85;
   const d=[c*hl+s*hd[0],hd[1]-.08*c*hd[0]];
   ha=angle([0,0],d);hsy=Math.hypot(...d)/7;
  }
  node('shoulder_'+side,'thorax',shoulder,{upper_angle:upper,depth:ds});node('elbow_'+side,'shoulder_'+side,elbow,{flex,local_rotation:-flex,depth:de});
  node('wrist_'+side,'elbow_'+side,wrist,{local_rotation:wristFlex,depth:dw});node('hand_'+side,'wrist_'+side,add(wrist,down(ha,7*hsy)),{angle:ha,depth:dw});
  arms[side]={upper_angle:upper,elbow_flex:flex,wrist_flex:wristFlex,depth:dw};
  const handMirror=view==='up_right'?1:(mirrored(side)?-1:1);
  const group=[joint('shoulder','shoulder_joint_'+side,shoulder,'thorax'),segment('upper_arm','upper_arm_'+side,shoulder,elbow,'shoulder_'+side),joint('elbow','elbow_joint_'+side,elbow,'shoulder_'+side),segment('forearm','forearm_'+side,elbow,wrist,'elbow_'+side),joint('wrist','wrist_joint_'+side,wrist,'elbow_'+side),part('hand','hand_'+side,wrist,ha,handMirror,hsy,'wrist_'+side)];
  const depths=[ds,(ds+de)/2,de,(de+dw)/2,dw,dw];
  group[0].scale=[.72,.72];
  group.forEach((o,i)=>o.depth=depths[i]);chains.push(['shoulder_'+side,'elbow_'+side,'wrist_'+side,'hand_'+side]);
  return group;
 }
 const legs=[leg('far'),leg('near')].sort((a,b)=>a.depth-b.depth);
 const armGroups=[arm('far'),arm('near')].sort((a,b)=>a[0].depth-b[0].depth);
 for(const l of legs)if(!l.foreground)ops.push(...l.ops);
 for(const group of armGroups)ops.push(...group.filter(o=>o.depth<0||o.asset==='shoulder'));
 ops.push(joint('waist','waist_joint',waist,'root'),part('lower_body','lower_body',waist,C.body.pelvis_roll[f]*c,1,1,'root'));
 for(const l of legs)if(l.foreground)ops.push(...l.ops);
 const base=add(cervical,[0,3.5]),mid=[(base[0]+neck[0])/2,(base[1]+neck[1])/2];
 ops.push(part('neck','neck',mid,angle(neck,base),.875,(Math.hypot(base[0]-neck[0],base[1]-neck[1])+4)/A.neck.size[1],'cervical'));
 ops.push(part('upper_body','upper_body',waist,lean,1,1,'waist'));
 for(const group of armGroups)ops.push(...group.filter(o=>o.depth>=0&&o.asset!=='shoulder'));
 ops.push(part('head','head',head,0,1,1,'neck'));
 chains.push(['sacrum','lumbar','thorax','cervical','neck'],['root','sacrum'],['shoulder_far','thorax','shoulder_near'],['hip_far','hip_near']);
 return {view,frame:f,ops,joints,chains,arms,body_bob:bob,yaw:V.yaw};
}
...
const api={makePose,draw,drawStage,labels,soleOffset,groundSupport};
