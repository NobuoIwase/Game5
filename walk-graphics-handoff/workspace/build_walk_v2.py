...
OUT = ROOT / 'lumina-walk-v2'
OUT.mkdir(exist_ok=True)
W,H = 192,256
N=8
DURATION=120
SOURCES = {
    'front': ROOT/'generated_images/exec-37d79d2b-5881-4da9-ba43-a76ee01b64f6.png',
    'side': ROOT/'generated_images/exec-ed7f4c50-1d27-4a6f-861f-55a8c59e0f2e.png',
}
...
def pose_front(f):
    ...
    swing=[-3,-2,0,2,3,2,0,-2][f]
    ops += [op('skirt',96+sway*.3,143+b*.6,(51,0),sway*.45)]
    # The shoulder roots are local to the torso. The upper arm joint is
    # covered by the bodice; forearms remain visible over the skirt.
    roll=sway*.45
    theta=math.radians(roll)
    for name,local_x,pivot,rest in [('arm_far',-14,(23,3),-12),('arm_near',14,(5,3),12)]:
        ax=96+sway+local_x*math.cos(theta)-13*math.sin(theta)
        ay=112+b+local_x*math.sin(theta)+13*math.cos(theta)
        ops.append(op(name,ax,ay,pivot,rest+swing+roll))
...
def whole_leg(name,hip,target,frame):
    ...
    def center(y):
        ...
        foot=joint_ankle+np.array([0.,y-yankle])
    ...
    def triangle(src,dest):
        ...
        opaque=samples[:,3]>0
        # Transparent texels must not erase an adjacent strip at an inner bend.
    ...
    return [op(key,hip[0],hip[1],tuple(origin))]
...
def pose_side(f):
    ...
    footy=[232,232,232,232,232,225,222,228]
    def leg(name,offset):
        k=(f+offset)%8
        return whole_leg(name,hip,(107+footx[k],footy[k]),f)
    ops=[op('wing_far',98,127+b,(42,27),lag),
         op('tail_far',107,181+b,(40,3),lag*1.7)]
    ops+=leg('leg_far',4)
    ops+=[op('arm_far',112,123+b,(8,3),-s*.7)]
    ops+=[op('hair',91,48+b,(42,0),lag*.8),
          op('tail_near',108,183+b,(44,3),lag*1.7)]
    ops+=leg('leg_near',0)
    ops += [op('skirt',110,144+b*.65,(40,0),lag*.5),
            op('torso',109,109+b,(17,0),0),
            op('wing_near',96,125+b,(44,27),lag*1.2),
            op('arm_near',113,123+b,(7,3),s*.7),
            op('head',96,27+b,(40,0),0)]
    return ops
...
preview[0].save(ROOT/'walk-check-v2.png')
...
(ROOT/'player-data-v2.json').write_text(json.dumps(DATA,separators=(',',':')))
template=ROOT/'player-template-v2.html'
if template.exists():
    (OUT/'lumina-walk-player.html').write_text(template.read_text().replace('__DATA__',json.dumps(DATA,separators=(',',':'))))
print(json.dumps({'output':str(OUT),'part_count':sum(len(p) for p in PARTS.values()),'frame_count':16,'preview':str(ROOT/'walk-check-v2.png')}))
