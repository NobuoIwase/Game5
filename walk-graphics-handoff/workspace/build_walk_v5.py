...
ARM_SOURCE=ROOT/'generated_images/exec-d8e8f8d6-9f3b-44ab-b5db-7521449674b5.png'
CLEANUP_SOURCE=ROOT/'generated_images/exec-610e3c07-1291-4781-b90a-c6485e7d89e0.png'
...
def extract():
    ...
    for view,path in SOURCES.items():
        ...
        for i,name in enumerate(NAMES):
            ...
            if view=='side' and name=='torso':
                ...
                original_piece.paste(piece.crop((0,0,piece.width,8)),(0,0))
    ...
    for i,(view,name,size) in enumerate([
        ...
        p=Image.fromarray(a)
        p=p.crop(p.getbbox()).resize(size,Image.Resampling.NEAREST)
        parts[view][name]=p
...
def whole_arm(name,shoulder,upper_angle,frame):
    ...
    controls_y=[3,16,26,41,49]
    controls_x=[src_center(y) for y in [3,16,26,41]]
    ...
    def center(y):
        ...
        return (1-t)*upper_pos+t*lower_pos
    def vertex(x,y):
        c=center(y)
...
preview[0].save(ROOT/'walk-check-v5.png')
...
(ROOT/'player-data-v5.json').write_text(json.dumps(DATA,separators=(',',':')))
template=ROOT/'player-template-v5.html'
if template.exists():
    (OUT/'lumina-walk-player.html').write_text(template.read_text().replace('__DATA__',json.dumps(DATA,separators=(',',':'))))
print(json.dumps({'output':str(OUT),'part_count':sum(len(p) for p in PARTS.values()),'frame_count':16,'preview':str(ROOT/'walk-check-v5.png')}))
