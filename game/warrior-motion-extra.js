...
function draw(c,h,a){let d=h.dir||'front',v=S.r?.views?.[d],o=imgs(d);if(!v||!o.body.ok)return 0;let p=pose(h,a),z=112/543,x=h.x-v.root[0]*z+p[5]*z,y=h.y-26-v.root[1]*z+p[6]*z;c.save();c.translate(x,y);c.scale(z,z);if(p[7])rot(c,v.root,p[7]);if(o.scabbard.ok)c.drawImage(o.scabbard,0,0);for(let l of v.legs||[])limb(c,o['leg_'+l.side],l.hip,l.side==='right'?p[3]:p[4]);let al=v.arms?.find(x=>x.side==='left'),ar=v.arms?.find(x=>x.side==='right');if(al)limb(c,o.arm_left,al.shoulder,p[1]);c.save();rot(c,v.root,p[0]);c.drawImage(o.body,0,0);c.restore();if(ar){c.save();rot(c,ar.shoulder,p[2]);if(o.sword.ok)c.drawImae(o.sword,0,0);if(o.arm_right.ok)c.drawImage(o.arm_right,0,0);c.restore()}if(o.shield.ok)c.drawImage(o.shield,0,0);c.restore();return 1}
