...
for(const view of ['front','side'])for(let f=0;f<8;f++){
 ...
 for(const side of ['far','near']){
  ...
  if(view==='side'){
   ...
   if(h.phase===0||h.phase===7)assert(a.position[0]>k.position[0],'Shin must extend ahead before heel strike');
  }
 }
...
assert(Math.max(...flex)-Math.min(...flex)>=30,'Elbow motion must not be rigid');
assert(Math.max(...flex)===flex[5],'Elbow flexion must peak after the forward upper-arm swing');
console.log('PASS: 16 poses × 29 parts, linked endpoints, limb lengths, 10–44° elbows, delayed elbow peak, shin extension.');
