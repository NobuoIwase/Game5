// Builds references/index.html: every attack and restrained-pose motion on one page, animated,
// with links to the frame sheets and the key-frame pages.
//   node tools/build_reference_index.mjs   (after render_attack_svg.mjs / render_restraint_svg.mjs)
...
const VIEW={front:'正面',up_right:'斜め後ろ',back:'後ろ',right:'右',down_right:'右斜め前'};
function cards(lib,dir,kind){
 return Object.entries(lib.motions).map(([id,m])=>{
  const n=m.frame_ms.length,tot=m.frame_ms.reduce((a,b)=>a+b,0),keys=kind==='attack'?['right','down_right']:(m.view==='front'?['front','right']:['up_right','back','right']);
...
const a=attackLib(),r=restraintLib();
...
<header><h1>Game5 モーション参考資料</h1><p>骨格から描いたマネキン図（青＝本人の右半身、橙＝左半身、紫＝拘束）。図をタップすると大きく開きます。依頼書：<a href="https://github.com/NobuoIwase/Game5/blob/main/character-motion-v1/ATTACK_MOTION_REQUEST.md">攻撃</a> · <a href="https://github.com/NobuoIwase/Game5/blob/main/character-motion-v1/RESTRAINT_MOTION_REQUEST.md">拘束された姿勢</a></p></header>
<nav><div class="in"><button data-f="all" aria-pressed="true">すべて</button><button data-f="attack" aria-pressed="false">攻撃（${Object.keys(a.motions).length}）</button><button data-f="restraint" aria-pressed="false">拘束された姿勢（${Object.keys(r.motions).length}）</button></div></nav>
<main>
