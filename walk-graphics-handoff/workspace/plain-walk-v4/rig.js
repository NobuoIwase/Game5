...
function makeTurnedPose(view,frame,C,assets,params={}){
 ...
 function leg(side){
  ...
  const fa=C.gait.foot_angle[k]*s*.32;
  const footScale=clamp(1+da*.0015,.94,1.035);
  const floor=V.ground_center+V.ground_depth*da;
  const ay=floor-lift-soleOffset(A[footAsset],fa)*footScale;
  ...
  node('ankle_'+side,'knee_'+side,ankle,{depth:da,contact:k<=4,foot_angle:fa,foot_pitch:C.gait.foot_angle[k],foot_asset:footAsset,ground_y:floor,lift,sagittal_position:a0.position});
  const thigh=segment('thigh','thigh_'+side,h,knee,'hip_'+side),shin=segment('shin','shin_'+side,knee,ankle,'knee_'+side);
 ...
 function arm(side){
  ...
  const wristFlex=C.arms.wrist_flex[k],lower=upper-flex,handAngle=lower+wristFlex;
  ...
  const shoulder=project(l,sy,z0),elbow=project(el,sy+es[1]-.08*c*es[0],z0+es[0]),wrist=project(wl,sy+ws[1]-.08*c*ws[0],z0+ws[0]);
  const ds=depth(l,z0),de=depth(el,z0+es[0]),dw=depth(wl,z0+ws[0]);
  const ha=handAngle*s+sign*c*(4-.18*wristFlex),hsy=1-Math.abs(c)*(1-clamp(Math.cos(rad(handAngle)),.64,1));
