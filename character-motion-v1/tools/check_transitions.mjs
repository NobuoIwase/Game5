...
import {library as R,MOTIONS as RM} from '../motion/restraint.mjs';import {library as S,MOTIONS as SM,NEUTRAL} from '../motion/scenes.mjs';
...
const L={...R().motions,...S().motions},K=m=>m==='stand'?[NEUTRAL]:(RM[m]||SM[m]).keys;
const last=m=>{const k=K(m);return pose(m==='stand'||L[m].loop?k[0]:k[k.length-1])},first=m=>pose(K(m)[0]);
