/** Restraint and H-scene motions for any character, on the same skeleton and projection as
 * poses.json / attack.mjs (the forward kinematics are attack.mjs's pose()).
 *
 * She faces the viewer while held (direction "front"), so the body is shown; the bent-over pose
 * that shows her from behind is meant for "back". Restraints are drawn as tentacle anchors:
 * each bind ties a joint to a point in the world (or two joints together, or runs a tentacle
 * between her legs through a via point) - the sprite artist draws the tentacles there.
 *
...
/* the spread (gani-mata) stance the tentacles force her into */
const SPREAD={hipY:196,footL:foot(22,2,{yaw:45,knee:1.1}),footR:foot(-22,2,{yaw:45,knee:1.1})};
const KNEE_TENTACLES=[{j:'knee_left',to:[50,212,8]},{j:'knee_right',to:[-50,212,8]}];
/* a tentacle between her legs, front to back, passing just under her (rubbing) */
const rod=k=>({j:'crotch',to:[0,206,48],via:[k.sway||0,k.hipY+9,(k.rootZ||0)+1],to2:[0,212,-48]});
/* one rising from the floor into her (the bounce) */
const pole=k=>({j:'crotch',to:[0,242,6],via:[k.sway||0,k.hipY+8,(k.rootZ||0)+2]});
const loop=(n,ms,f)=>Array.from({length:n},(_,i)=>({...f(i/n,i),ms}));

export const MOTIONS={
 /* ---- arms tied behind her back ---- */
 arms_behind_squirm:{label:'後ろ手拘束・身をよじる',loop:true,view:'front',keys:loop(8,120,t=>({...BASE,
  ...
  phase:'hips sway and twist, knees rub together, one heel then the other comes up; she looks down and away'}))},
 arms_behind_wrench:{label:'後ろ手拘束・力で抜けようとする',loop:true,view:'front',keys:[0,1,2,3,4,5,6,7].map(i=>{
 ...
 elbows_up_strain:{label:'肘を曲げ前腕を立てた拘束・引き下ろそうと力む',loop:true,view:'front',keys:loop(8,110,(t)=>{
  ...
  return{...BASE,hipY:181+4*p,pitch:6*p-4*up,shrug:-2*p+2*up,sway:2*(pl-pr),torso:6*(pr-pl),
   ...
   phase:'pulls one arm down against the tentacle, then the other; hunches and bends her knees as she strains, is drawn up onto her toes when it wins'}})},
 /* ---- knees pressed together, forced open ---- */
 legs_forced_open:{label:'内股を触手でガニ股に開かれる',loop:false,view:'front',keys:[
  [10,-.3,-14,181,0,[-10,0,0],0,130,'閉じる knees pressed together, toes turned in'],
  [10,-.3,-16,182,4,[-16,0,0],1,110,'締める tentacles coil round her knees; she clamps them tighter, hunching'],
  ...
  [23,1.12,48,197,-2,[-2,0,0],1,140,'開かれた forced wide: knees splayed, hips low (gani-mata)'],
  ...
  [23,1.1,48,197,-2,[-4,0,0],1,160,'']].map(([w,kn,yw,hy,pt,hd,bd,ms,ph])=>({...BASE,hipY:hy,pitch:pt,
   footL:foot(w,2,{yaw:yw,knee:kn}),footR:foot(-w,2,{yaw:yw,knee:kn}),head:hd,binds:[WRIST_BAND,...(bd?KNEE_TENTACLES:[])],ms,phase:ph||'…'}))},
 spread_hold_tremble:{label:'ガニ股に開かれたまま耐える',loop:true,view:'front',keys:loop(8,90,t=>({...BASE,...SPREAD,
  ...
  head:[-6+4*C(t),5*S(t),0],binds:[WRIST_BAND,...KNEE_TENTACLES],phase:'held open: the knees quiver, the hips shiver'}))},
 /* ---- rubbing: hips pushed forward and back ---- */
 rub_thrust:{label:'素股・腰を前後に突き出す（ガニ股）',loop:true,view:'front',keys:loop(8,85,t=>{const k={...BASE,...SPREAD,
  rootZ:6*S(t),hipY:195-1.5*C(t),pitch:-7*S(t),footL:foot(22,2,{yaw:45,knee:1.05}),footR:foot(-22,2,{yaw:45,knee:1.05}),
  head:[-3-5*S(t-.1),0,0],phase:S(t)>.3?'hips thrust forward, shoulders back':S(t)<-.3?'hips drawn back, chest forward':'…'};
  k.binds=[WRIST_BAND,...KNEE_TENTACLES,rod(k)];return k})},
 rub_thrust_closed:{label:'素股・内股で挟んで腰を揺らす',loop:true,view:'front',keys:loop(8,95,t=>{const k={...BASE,
  ...
  head:[-8-4*S(t-.1),3*S(t),0],phase:'thighs clamped on it; small thrusts, rising onto her toes each time'};
  k.binds=[WRIST_BAND,rod(k)];return k})},
 /* ---- taken: up and down ---- */
 ride_bounce:{label:'挿入・上下に揺さぶられる（ガニ股）',loop:true,view:'front',keys:loop(8,80,t=>{const d=.5-.5*C(t),k={...BASE,...SPREAD,
  ...
  k.binds=[WRIST_BAND,...KNEE_TENTACLES,pole(k)];return k})},
 ride_bounce_arms_up:{label:'挿入・上下に揺さぶられる（腕を吊られて）',loop:true,view:'front',keys:loop(8,75,t=>{const d=.5-.5*C(t),k={...BASE,...SPREAD,
  ...
  head:[8*(1-d)-8*d,4*S(t),0],phase:'hung by her wrists, bounced; the arms stretch as she drops'};
  k.binds=[{j:'wrist_right',to:[-37,64,4]},{j:'wrist_left',to:[37,64,4]},...KNEE_TENTACLES,pole(k)];return k})},
 /* ---- climax ---- */
 climax_toes:{label:'絶頂・爪先をピンと伸ばす',loop:false,view:'front',keys:[
  [194,1.0,0,0,[-4,0,0],110,'高まり tension rising, knees locked open'],
  ...
  [184,.62,60,-6,[14,4,0],60,'痙攣 jerk'],[182,.5,68,-10,[20,-4,0],60,'jerk'],[184,.64,60,-6,[14,4,0],60,'jerk'],[182,.52,66,-9,[18,-3,0],70,'jerk'],[185,.66,56,-5,[12,2,0],80,'…'],
  [192,1.0,20,6,[-10,0,0],120,'抜ける release: heels drop, knees give'],
  [197,1.12,0,10,[-18,0,0],150,'崩れる sags into the tentacles'],
  [198,1.15,0,12,[-22,4,0],240,'余韻 limp']].map(([hy,kn,pt,pi,hd,ms,ph])=>({...BASE,...SPREAD,hipY:hy,pitch:pi,
   footL:foot(22,2,{yaw:45,knee:kn,point:pt}),footR:foot(-22,2,{yaw:45,knee:kn,point:pt}),head:hd,binds:[WRIST_BAND,...KNEE_TENTACLES],ms,phase:ph}))},
 climax_arch:{label:'絶頂・のけぞり',loop:false,view:'front',keys:[
  ...
  [8,186,-32,-.2,35,[45,0,0],4,160,'頂点 PEAK: arched right back, head thrown back, on her toes'],
  [7,185,-28,-.25,30,[40,5,0],3,60,'痙攣 jerk'],[9,187,-34,-.18,36,[48,-5,0],4,60,'jerk'],[7,185,-29,-.25,30,[41,4,0],3,60,'jerk'],[9,186,-33,-.2,35,[46,-4,0],4,70,'jerk'],
  ...
  [0,193,18,-.66,0,[-30,6,0],0,240,'余韻 limp']].map(([rz,hy,pi,kn,pt,hd,sh,ms,ph])=>({...BASE,rootZ:rz,hipY:hy,pitch:pi,shrug:sh,
 ...
 /* ---- bent over, shown from behind (use the "back" view) ---- */
 show_rear:{label:'尻を突き出す前屈み拘束（後ろ・斜め後ろから見せる）',loop:true,view:'up_right',keys:loop(8,110,t=>({...BASE,
  ...
  phase:'bent right over, hips up and back; the hips wiggle side to side; she looks back over her shoulder'}))}
};
...
export function library(){
 ...
 return{schema:'anatomical-eight-direction-motion/1.0',extends:'poses.json / attack-poses.json (same skeleton, 288x288 canvas, centre x 144, projection)',
  ...
  notes:{view:'the direction the scene is shown in: front (facing the viewer) for all but show_rear, which is shown from behind (up_right = 3/4 from behind reads best; back also works)',
   binds:'each frame lists its restraints: joint (+ joint2 = a band between two joints) or joint -> anchor (a tentacle pulling toward a point), with via/anchor2 = a tentacle running from anchor through via to anchor2',
   extra_joints:{face:'9px ahead of the head centre: where she looks',crotch:'just under the hips'}},
  motions:meta,poses};
}
