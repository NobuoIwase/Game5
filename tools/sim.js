...
function runOnce(seed){
  ...
  return{win:s.over&&!h.dead&&!!s.dungeon?.complete,timeout:!s.over,room:(s.dungeon?.room||0)+1,time:+t.toFixed(1),
    ...
    pending:!!s.dungeon?.pending,hp:ctx.__hpLog,hits:ctx.__hits,sp:ctx.__spLog,hero:{x:Math.round(h.x),y:Math.round(h.y),thought:h.thought}};
}
