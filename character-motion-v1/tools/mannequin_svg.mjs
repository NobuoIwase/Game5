...
export function figure(J,binds=[],yaw=0,expr=''){   // expr: 'shut' closes the eyes; 'o' opens the mouth, 'line' presses it shut
 ...
 const place=(d,w)=>{if(!w)return d;const s=side(w);if(s<-3)return faces?Math.min(d,TD-1-.01*Math.min(40,-s)):Math.max(d,TD+1);if(s>3)return faces?Math.max(d,TD+1):Math.min(d,TD-1);return d};
 ...
 const seg=(a,b,w,col)=>items.push({d:isArm.test(a)?place((D(a)+D(b))/2,mid(a,b)):(D(a)+D(b))/2,svg:line(P(a),P(b),w,col)});
 for(const s of ['right','left']){const c=C[s];
  seg('hip_'+s,'knee_'+s,9,c);seg('knee_'+s,'ankle_'+s,7,c);
  {const a=P('ankle_'+s);items.push({d:(D('ankle_'+s)+D('toe_'+s))/2,svg:line([a[0],a[1]+2],P('toe_'+s),7,c)})}
  items.push({d:D('knee_'+s)+.02,svg:ell(P('knee_'+s),4,4,C.knee,1.5)});
  seg('shoulder_'+s,'elbow_'+s,7,c);seg('elbow_'+s,'wrist_'+s,6,c);
  items.push({d:place(D('wrist_'+s)+.02,W('wrist_'+s)),svg:ell(P('wrist_'+s),4,5,c,1.5)});
 }
 ...
 {const t=P('thorax'),r=P('root'),v=[t[0]-r[0],t[1]-r[1]],l=Math.hypot(...v)||1,u=[v[0]/l,v[1]/l],n=[-u[1],u[0]];
  ...
  items.push({d:TD-.01,svg:`<polygon points="${q.map(p=>p.map(f).join(',')).join(' ')}" fill="${C.body}" stroke="${C.out}" stroke-width="2" stroke-linejoin="round"/>`+ell(r,10,7,C.pelvis)});
...
export function sheet(lib,m,dirs,{mark}={}){
 ...
 dirs.forEach((d,r)=>{s+=`<text x="8" y="${pad+r*ch+ch/2}" font-size="16" fill="${C.text}">${LABEL[d]}</text><text x="8" y="${pad+r*ch+ch/2+18}" font-size="12" fill="${C.sub}">${d}</text>`;
  for(let i=0;i<N;i++){const fr=P[d][i];s+=`<g transform="translate(${lw+i*cw},${pad+r*ch})">${cell(cw,ch,i===hit)}${figure(fr.joints,fr.binds,fr.yaw,fr.expr)}</g>`}});
 return s+'</svg>';
}
...
export function keys(lib,m,d,{trail='sword_tip',mark}={}){
 ...
 for(let i=0;i<N;i++){const fr=F[i];
  s+=`<g transform="translate(${i*cw},0)">${cell(cw,ch,i===hit)}`+
   ...
   `${figure(fr.joints,fr.binds,fr.yaw,fr.expr)}</g>`;
...
export function preview(lib,m,dirs,cols=4){
 ...
 dirs.forEach((d,r)=>{const x=(r%cols)*cw,y=Math.floor(r/cols)*(ch+10);s+=`<g transform="translate(${x},${y})">${cell(cw,ch,false)}<text x="8" y="18" font-size="13" fill="${C.text}">${LABEL[d]}</text>`;
  for(let i=0;i<N;i++){const vals=ms.map((_,k)=>k===i?'inline':'none').join(';'),fr=P[d][i];
   s+=`<g display="${i===0?'inline':'none'}">${figure(fr.joints,fr.binds,fr.yaw,fr.expr)}<animate attributeName="display" values="${vals}" keyTimes="${kt.map(v=>v.toFixed(4)).join(';')}" calcMode="discrete" dur="${tot}ms" repeatCount="indefinite"/></g>`}
  s+='</g>'});
 return s+'</svg>';
}
