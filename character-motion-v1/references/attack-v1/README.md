...
`motion/attack.mjs` の骨格を `tools/render_attack_svg.mjs` で描いたマネキン図（v3：女性の剣士の動き）。
...
`<motion>` は次の7つ。
- `slash`（通常斬り＝踏み込み振り下ろし）
- `kesa`（袈裟懸け＝右上から左下への斜め斬り）
- `yoko`（横一文字＝右から左への水平斬り）
- `thrust`（突き）
- `heavy`（破城斬り＝跳び上がって叩きつけ）
- `advance`（構えたまま前進）
- `retreat`（構えたまま後退）

横一文字は、横からより斜め前（`_keys_down_right.svg`）から見る方が分かりやすい。
